...
OUTPUT_XLSX = "indialawacts_sections_dropdown_audit.xlsx"
USER_AGENT = (
...
def create_workbook(
    ...
    ws = wb.active
    ws.title = "All_Acts"
    ws.append(["Section", "Clause", "Description"])
    for rec in all_records:
        ws.append([rec.combined_section_display, rec.clause, rec.description])
    style_header(ws)
    style_data(ws, 2, ws.max_row, 1, 3)
    ws.freeze_panes = "A2"
    ...
    for act_name, records in per_act_records.items():
        ...
        aws.append(["Section", "Clause", "Description"])
        for rec in records:
            aws.append([rec.section_display, rec.clause, rec.description])
        style_header(aws)
        style_data(aws, 2, aws.max_row, 1, 3)
        aws.freeze_panes = "A2"
        aws.auto_filter.ref = f"A1:C{aws.max_row}"
        aws.column_dimensions["A"].width = 44
        aws.column_dimensions["B"].width = 95
        aws.column_dimensions["C"].width = 120
